    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 py-6 backdrop-blur-sm">
      <div ref={dialogRef} className={`w-full max-w-3xl rounded-[28px] border border-white/10 bg-slate-900/95 p-6 shadow-2xl ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.35em] text-slate-400">{t('customers.create_title')}</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">{t('customers.create_subtitle')}</h2>
          </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-200">{t('customers.label.full_name')}</label>
            <input
              {...register('full_name')}
              ...
              placeholder={t('customers.placeholder.full_name')}
            />
            {errors.full_name ? <p className="mt-2 text-sm text-rose-400">{errors.full_name.message}</p> : null}
          </div>

          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-200">{t('customers.label.phone')}</label>
            <input
              {...register('phone')}
              className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-200">{t('customers.label.email')}</label>
            <input
              {...register('email')}
              className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-200">{t('customers.label.address')}</label>
            <input
              {...register('address')}
              className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold text-slate-200">{t('customers.label.city')}</label>
            <input
              {...register('city')}
              className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-5 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={isSubmitting} className="rounded-[16px] bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60">{isSubmitting ? t('common.button.processing') : t('customers.button.save')}</button>
          </div>
        </form>
      </div>
    </div>
