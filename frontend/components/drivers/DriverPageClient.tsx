    ...
    <>
      <div className="flex flex-col gap-4 rounded-[28px] border border-white/10 bg-slate-900/70 p-6 shadow-2xl shadow-slate-950/40 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white">Manage drivers</h1>
          <p className="mt-2 text-sm text-slate-400">Create and manage delivery drivers across branches.</p>
        </div>

        <div className="flex items-center gap-3">
          <div className="rounded-[16px] border border-white/10 bg-slate-800/80 px-4 py-2 text-sm text-slate-300">{summary.total} drivers</div>
      ...
      <div className="mt-6">
        {isLoading ? (
          ...
          <p className="text-rose-400">Unable to load drivers</p>
        ) : (
