...
const shipmentFormSchema = (t: (key: string) => string) => z.object({
  sender_name: z.string().trim().min(2, t('shipments.errors.sender_name_required')).max(100),
  receiver_name: z.string().trim().min(2, t('shipments.errors.receiver_name_required')).max(100),
  receiver_phone: z.string().trim().regex(/^\d{11}$/, t('shipments.errors.receiver_phone_invalid')),
  address: z.string().trim().min(5, t('shipments.errors.address_required')).max(255),
  city: z.string().trim().min(2, t('shipments.errors.city_required')).max(100),
...
type ShipmentFormValues = z.infer<ReturnType<typeof shipmentFormSchema>>
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit(submitHandler)}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.sender_name')}</label>
          <input
            ...
            placeholder={t('shipments.placeholder.sender_name')}
          />
          {showFieldError('sender_name') ? <p className="mt-2 text-sm text-rose-400">{errors.sender_name?.message}</p> : null}
        </div>

        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.receiver_name')}</label>
          <input
            {...register('receiver_name')}
            className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
            placeholder={t('shipments.placeholder.receiver_name')}
          />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.receiver_phone')}</label>
          <input
            {...register('receiver_phone')}
            className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
            placeholder={t('shipments.placeholder.receiver_phone')}
          />
          {showFieldError('receiver_phone') ? <p className="mt-2 text-sm text-rose-400">{errors.receiver_phone?.message}</p> : null}
        </div>

        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.city')}</label>
          <input
            {...register('city')}
            className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
            placeholder={t('shipments.placeholder.city')}
          />
      ...
      <div>
        <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.address')}</label>
        <textarea
          {...register('address')}
          rows={3}
          ...
          placeholder={t('shipments.placeholder.address')}
        />
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.status')}</label>
          <select
            {...register('status')}
            className="h-12 w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 text-sm text-white outline-none transition focus:border-sky-400"
          >
        ...
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.estimated_delivery_days')}</label>
          <input
            type="number"
            min={1}
            max={365}
        ...
        <div>
          <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.cod_amount')}</label>
          <input
            type="number"
            min={0}
            step="0.01"
      ...
      <div>
        <label className="mb-2 block text-sm font-semibold text-slate-200">{t('shipments.label.notes')}</label>
        <textarea
          {...register('notes')}
          rows={3}
          className="w-full rounded-[16px] border border-white/10 bg-slate-900/70 px-4 py-3 text-sm text-white outline-none transition focus:border-sky-400"
          placeholder={t('shipments.placeholder.notes')}
        />
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-5 sm:flex-row sm:justify-end">
        ...
        <button type="submit" disabled={isSubmitting} className="rounded-[16px] bg-sky-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-60">
          {isSubmitting ? (mode === 'edit' ? t('shipments.button.saving') : t('shipments.button.creating')) : resolvedSubmitLabel}
        </button>
      </div>
    </form>
  )
